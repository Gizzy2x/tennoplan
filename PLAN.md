...
## Plan to address API error and tsconfig warning
### Goal
To resolve the `500 error on /api/worldstate` by enhancing error handling, adding necessary headers, and fixing a deprecated warning in `tsconfig.app.json`.
### Steps
1.  **Examine `api/worldstate.ts`**: Read the file to understand the current fetch request implementation.
2.  **Examine `tsconfig.app.json`**: Read the file to identify the deprecated `baseUrl` configuration.
3.  **Modify `api/worldstate.ts`**:
    *   Add a `User-Agent` header to the fetch request.
    *   Implement comprehensive error handling to log the raw response text when the fetch fails (e.g., due to `Unexpected end of JSON input`).
    *   Add a Vercel cache header (`s-maxage=300`) to the response.
4.  **Modify `tsconfig.app.json`**: Correct the deprecated `baseUrl` configuration.
5.  **Seek User Approval**: Present the detailed plan to the user for review and approval.
6.  **Switch to `Code` Mode**: Once approved, switch to `code` mode to implement the planned changes.