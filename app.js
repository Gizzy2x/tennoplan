...
const today = state[DAY_KEY];

// ── Pulse migration ──
// Converts any prior storage format to the new flat boolean model.
...
function migratePulseWeek(w) {
  if (w.runs && 'edaNormal' in w.runs) return false; // already flat
  const r = w.runs || {};

  // Read tiered v2 format
  ...
  w.runs = { edaNormal:edaN, edaElite:edaE, taNormal:taN, taElite:taE,
...
// ── Reward panel data ──
// Hardcoded reward lists per card id. sortie also tries getDropsFor() live (capped at 8).
const CARD_REWARDS = {
  archon: {
...
function buildRewardPanelHTML(cardId) {
  ...
  if (liveUsed) {
    const header = `<div class="rewards-header"><span class="rh-item">Item</span><span>Rarity</span><span>Chance</span></div>`;
    return header + rows + (def.note ? `<div class="reward-note">${def.note}</div>` : '');
  }
  const chips = def.items.map(item =>
    `<div class="reward-chip"><span class="reward-chip-name">${item}</span></div>`
  ).join('');
...
// Toggle reward panel on pulse budget cards (called from inline onclick in HTML)
function togglePulseRewardPanel(key, btn) {
...
function makeCard(t, taskStore) {
  ...
  if (hasRewards) {
    ...
    toggle.addEventListener('click', e => {
      ...
      if (openRewardPanels.has(t.id)) {
        ...
        const panel = div.querySelector('.card-rewards-panel');
  ...
  div.onclick = () => { store[t.id] = !store[t.id]; div.classList.toggle('done', !!store[t.id]); persist(); };
  return div;
}
...
// ── Pulses ──
// Each key in week.runs is exactly 1 pulse — count is simply the number of true values.
function countPulsesUsed() {
  return Object.values(week.runs).filter(Boolean).length;
}

// EDA / Temporal: Normal + optional Elite tier
// Cascade rules: turning Elite ON auto-sets Normal first; turning Normal OFF auto-clears Elite first.
function toggleTieredRun(mode, tier) {
  ...
  if (tier === 'elite') {
    if (r[ek]) {
      r[ek] = false; // turn elite off; normal stays
    } else {
      // Cascade: elite on requires normal — cost is 1 if normal already set, 2 if not
    ...
    if (r[nk]) {
      // Cascade: turning normal off first clears elite, then clears normal
      r[ek] = false;
      r[nk] = false;
    } else {
...
// Netracells: each run is a flat boolean key net1–net5
...
function renderPulses() {
  ...
  // Netracells — read flat keys net1–net5
...
// ── Drop data ──
// dropData is populated lazily by loadDropData() (called non-blocking from init).
// Cache: localStorage keys 'wf-drops-cache' (JSON) + 'wf-drops-cache-ts' (ms timestamp), TTL 24h.
// Data shape: flat array of {place, item, rarity, chance} — ~40 000 entries.
// Query interface: getDropsFor(itemName) — returns [{place,item,rarity,chance}] or null.
function getDropsFor(itemName) {
...
function applyArbitrationCard(arb) {
  ...
  // Wire up arbitration rewards toggle (re-wired on every re-render)
  ...
  if (arbToggle) {
    ...
    // Restore panel if it was open before re-render
    if (openRewardPanels.has('arbitration')) {
      const panel = document.createElement('div');
      panel.className = 'card-rewards-panel';
      panel.innerHTML = buildRewardPanelHTML('arbitration');
...
function updateCountdown() {
  ...
  // Weekly reset — next UTC Monday 00:00
  const day = now.getUTCDay();
  ...
  const weeklyStr = `${String(wh).padStart(2,'0')}:${String(wm).padStart(2,'0')}:${String(ws).padStart(2,'0')}`;
  const tw = document.getElementById('timer-weekly'); if (tw) tw.textContent = weeklyStr;
...
async function init() {
  ...
  function renderVoidFissures(fissures) {
    ...
    const byTier = {};
    filtered.forEach(f => {
      if (!byTier[f._tier]) byTier[f._tier] = [];
      byTier[f._tier].push(f);
    });
    const extraTiers = Object.keys(byTier).filter(t => tierOrder.indexOf(t) === -1).sort();
    const tiersToShow = tierOrder.concat(extraTiers).filter(t => byTier[t] && byTier[t].length);
    ...
    const stormByTier = {};
    storms.forEach(f => {
      if (!stormByTier[f._tier]) stormByTier[f._tier] = [];
      stormByTier[f._tier].push(f);
    });
    const stormExtraTiers = Object.keys(stormByTier).filter(t => tierOrder.indexOf(t) === -1).sort();
    const stormTiersToShow = tierOrder.concat(stormExtraTiers).filter(t => stormByTier[t] && stormByTier[t].length);
    ...
    if (!stormTiersToShow.length) {
      ...
      stormTiersToShow.forEach(tierName => {
  ...
  function renderInvasions(data) {
    ...
    // Sort by soonest expiry — most urgent invasions first
    active.sort((a, b) => new Date(a.expiry || 0) - new Date(b.expiry || 0));

    const list = document.createElement('div');
    list.className = 'inv-list';

    // Display name map for known rewardType keys
    const REWARD_NAMES = {
      ...
      mutalist:  'Nav Coordinate',
    };
    function titleCase(s) {
      return s.replace(/\w\S*/g, w => w.charAt(0).toUpperCase() + w.slice(1).toLowerCase());
    }
    // Resolve a rewardType key to a display name.
    function resolveRewardName(key, inv) {
      const k = String(key).toLowerCase();
      const items = (inv.attacker && inv.attacker.reward && Array.isArray(inv.attacker.reward.countedItems))
        ? inv.attacker.reward.countedItems : [];
      const found = items.find(ci => ci && String(ci.type || '').toLowerCase().includes(k));
      if (found && found.type) return found.type;
      return REWARD_NAMES[k] || titleCase(key);
    }
    ...
    active.forEach(inv => {
      ...
      row.innerHTML = `
        ...
        <div style="margin-top:4px">
          ...
          <div class="inv-reward-pills">
            ${rewards.map(r => `<span class="inv-reward-pill">${r}</span>`).join('')}
          </div>
        </div>` : ''}
    ...
    // Shared tick for invasion countdown timers
    if (!invasionTimerOn) {
  ...
  // Load drop data non-blocking — never touches badge or blocks init.
  async function loadDropData() {
    const CACHE_KEY    = 'wf-drops-cache';
    const CACHE_TS_KEY = 'wf-drops-cache-ts';
    const TTL          = 24 * 60 * 60 * 1000;
    try {
      // Clear a broken empty-array cache so it re-fetches fresh
      ...
      } catch (_) { /* storage full — cached in memory only */ }
    } catch (_) { /* silent fail — drop data is optional */ }
  }
  loadDropData();
}
